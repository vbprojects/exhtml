...
int forward(char* line, int line_num, int max_line_length, char* line_out, char line_tags[][100]){
    ...
    int pchrl = strlen(pchr);
    char pchb[pchrl + 1];
    char* pch = (char*) ((void*) pchb);
    strncpy(pch, pchr, pchrl + 1);
    pch[strcspn(pch,"\n\r")] = 0;
    int pchl = strlen(pch);
    
    bool user_specified_tag = pch[0] == '<';
    bool self_closing = pch[pchl - 1] == '>';

    printf("Tag: %s %d %d\n", pch, pchl, is_tag(pch, pchl) || is_tag(pch, pchl - 1) || user_specified_tag);
    if(user_specified_tag){
        char temp[pchl];
        strncpy(temp, pch, pchl);
        memset(pch, 0, pchl);
        strncpy(pch, temp + 1, pchl - 1);
    }
    if(self_closing){
        pch[pchl - 1] = '\0';
    }
    // printf("Forward Start Line: %s", line);
    if(is_tag(pch, pchl) || is_tag(pch, pchl - 1) || user_specified_tag){
        ...
        if(self_closing){
            sprintf(line_out, "%s/>", line_out, line_tags[line_num]);
            strncpy(line_tags[line_num],"\0",1);
        }else{
            sprintf(line_out, "%s>", line_out);
        }
    }
    else{
        // strncpy(line_tags[line_num],"\0",1);
